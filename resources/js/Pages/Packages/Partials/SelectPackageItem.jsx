...
export const SelectPackageItem = ({item}) => {
    ...
    return (
        <label role='button' className="bg-white w-full rounded-xl shadow-md p-3 text-muted flex items-center space-x-3">
            ...
            <div class={'flex-1'}>
                <p class={'font-medium'}>{item.test_package_name}</p>
                <p class={'text-gray-400'}>NGN {item.test_package_price}</p>
            </div>
            <div class={'shrink-0'}>
                <button onClick={() => add(item)} className="w-10 h-10 rounded-full flex items-center justify-center bg-primary">
